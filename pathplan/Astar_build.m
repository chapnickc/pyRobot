...
%optimal_path = pathAStar( StartX, StartY, MAP, GoalRegister, Connecting_Distance)
...
StartY= startpoint(2);
MAP = dilated;
GoalRegister = zeros(size(dilated));
...
%Note: If Hn values is set to zero the method will reduce to Dijkstras algorithm.
% Initializing start node with FValue and opening first node.
